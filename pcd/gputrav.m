function [i, t] = gputrav(origin, ray, xgv, ygv, zgv)
% GPUTRAV GPU-enabled ray tracing in voxel grid.
...
%      ray = gpuArray([-10, 0, 0]);
%      gv = gpuArray(-2:2);
%      xgv = gv; ygv = gv; zgv = gv;
%      [i, t] = gputrav(origin, ray, xgv, ygv, zgv)
%
%   See also TRAV, SLAB.
...
i = NaN(sum(gridsize), 3, 'gpuArray'); 
t = NaN(sum(gridsize), 1, 'gpuArray');
...
% Define index of the end of the payload data of the return matrices.
iEnd = 0;
tEnd = 0;
